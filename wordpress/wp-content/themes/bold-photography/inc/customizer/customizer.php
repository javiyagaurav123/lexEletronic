 ...
 */
function bold_photography_customize_register( $wp_customize ) {
	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial( 'blogname', array(
			...
			'render_callback' => 'bold_photography_customize_partial_blogname',
		) );
		$wp_customize->selective_refresh->add_partial( 'blogdescription', array(
			'selector' => '.site-description',
			'container_inclusive' => false,
			'render_callback' => 'bold_photography_customize_partial_blogdescription',
		) );
	}

	// Important Links.
	$wp_customize->add_section( 'bold_photography_important_links', array(
		'priority'      => 999,
		'title'         => esc_html__( 'Important Links', 'bold-photography' ),
	) );

	// Has dummy Sanitizaition function as it contains no value to be sanitized.
	bold_photography_register_option( $wp_customize, array(
			'name'              => 'bold_photography_important_links',
			'sanitize_callback' => 'sanitize_text_field',
			'custom_control'    => 'Bold_Photography_Important_Links_Control',
			'label'             => esc_html__( 'Important Links', 'bold-photography' ),
			'section'           => 'bold_photography_important_links',
			'type'              => 'bold_photography_important_links',
		)
	);
	// Important Links End.
}
add_action( 'customize_register', 'bold_photography_customize_register', 11 );

/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function bold_photography_customize_preview_js() {
	wp_enqueue_script( 'bold-photography-customize-preview', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'assets/js/customize-preview.min.js', array( 'customize-preview' ), '20180103', true );
}
add_action( 'customize_preview_init', 'bold_photography_customize_preview_js' );
